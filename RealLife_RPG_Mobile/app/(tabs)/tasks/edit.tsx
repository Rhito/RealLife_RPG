...
import { updateTask, TaskType, TaskDifficulty } from '../../../services/tasks';
...
export default function EditTaskScreen() {
    ...
    useEffect(() => {
        if (params.taskData) {
            try {
                const task = JSON.parse(params.taskData as string);
                setTaskId(task.id);
                setTitle(task.title);
                setDescription(task.description || '');
                setType(task.type);
                setDifficulty(task.difficulty);
                if (task.repeat_days) setRepeatDays(task.repeat_days);
    ...
    const handleSave = async () => {
        ...
        try {
            await updateTask(taskId, {
                ...
                type,
                difficulty,
                repeat_days: type === TaskType.TODO ? [] : repeatDays,
                due_date: type === TaskType.TODO ? date.toISOString() : undefined,
            } as any);
    ...
    const getTypeColor = (t: TaskType) => {
        switch(t) {
            case TaskType.HABIT: return Colors.rpg.accent;
            case TaskType.DAILY: return '#9E9E9E';
            case TaskType.TODO: return Colors.rpg.danger;
            default: return '#ccc';
        }
    };
        ...
        <View style={styles.mainContainer}>
            ...
            <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 40 }}>
                ...
                <TouchableOpacity style={styles.createButton} onPress={handleSave} disabled={loading}>
                    {loading ? <ActivityIndicator color="white" /> : <Text style={styles.createButtonText}>Save Changes</Text>}
                </TouchableOpacity>
