                ...
                <div class="kt-portlet kt-portlet--last kt-portlet--head-lg kt-portlet--responsive-mobile"
                    ...
                    <div class="kt-portlet__head kt-portlet__head--lg" style="">
                        <div class="kt-portlet__head-label">
                            <h3 class="kt-portlet__head-title">{{__('lang.edit')}} {{ $module['label'] }}
                            </h3>
                        </div>
                                ...
                                @foreach($module['form']['general_tab'] as $field)
                                    ...
                                    <div class="form-group-div form-group {{ @$field['group_class'] }}"
                                        ...
                                        @if($field['type'] == 'custom')
                                            @include($field['field'], ['field' => $field])
                                        @else
                                            <label for="{{ $field['name'] }}">{{ (isset($field['label_display_none']) &&  $field['label_display_none']) ? '' : __(@$field['label']) }} @if(strpos(@$field['class'], 'require') !== false)
                                                    <span class="color_btd">*</span>@endif</label>
                                            <div class="col-xs-12">
                                                @if($field['name'] == 'category_id')
                                                    @php
                                                        $data1 = App\Http\Helpers\CommonHelper::getCate();
                                                    @endphp
                                                    <select name="{{$field['name']}}" class="form-control" id="mySelect" style="overflow-y: scroll;">
                                                        <option value="">{{__('lang.select_category')}}</option>
                                                        @foreach($data1 as $item)
                                                            <option value="{{$item->id}}" @if($item->id == $field['value']) selected @endif >{{$item->name}}</option>
                                                        @endforeach
                                                    </select>
                                                    <script >
                                    ...
                                    @if($field['name'] == 'multi_cat')
                                        <div class="form-group-div form-group"
                                             id="form-group-{{ $field['name'] }}">
                                            <label for="{{ $field['name'] }}" style="font-size: 1.2rem;font-weight: bold;color: #48465b;">{{__('lang.product_classification')}}</label>
                                            <iframe id="iframe" src="/admin/product_attributes?product_id={{ $result->id }}"
