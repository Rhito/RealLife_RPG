...
{{--                                        @if(in_array($module['code'] . '_delete', $permissions))--}}
                                        ...
                                        <li class="kt-nav__item">
                                            <a href="javascript:void(0);" class="kt-nav__link" title="{{__('lang.Delete_all_row')}}" onclick="deleteAllRecords()">
                                                <i class="kt-nav__link-icon la la-copy"></i>
...
{{--đây là js để chỉnh hàng loạt  --}}
                    <script>
                        let selectedIds = [];

                        $(document).on("change", ".ids, .checkbox-master", function () {
                            if ($(this).hasClass("checkbox-master")) {
                                // Nếu click chọn tất cả
                                $(".ids").prop("checked", $(this).prop("checked"));
                            }

                            // Cập nhật lại mảng selectedIds
                            selectedIds = $(".ids:checked").map(function () {
                                return $(this).val();
                            }).get();

                            console.log("IDs được chọn qua checkbox:", selectedIds);
                        });

                        // Khi click vào nút Edit riêng lẻ
                        $(document).on('click', '.btn-edit-single', function () {
                            let id = $(this).data('id');
                            ...
                            console.log("IDs sau khi bấm Edit:", selectedIds);

                            // Hiện modal sửa hàng loạt
                        ...
                        function editAllRecords() {
                            ...
                            let formData = {
                                ids: selectedIds,
                                gia_ban: $("#gia_ban").val(),
                                gia_giam_gia: $("#gia_giam_gia").val(),
                            };


                            fetch("{{ route('shop_products.update_hang_loat') }}", {
                                method: "POST",
                                headers: {
                                    "Content-Type": "application/json",
                                    "X-CSRF-TOKEN": "{{ csrf_token() }}"
                                ...
                                .then(response => response.json())
                                .then(data => {
                                    $("#editModal").modal("hide"); // Đóng modal (Bootstrap 4)
                    ...
                    <div class="my-2">
                        ...
                        <a href="#" class="kt-nav__link" onclick="multiDelete();" title="{{__('lang.Delete_all_selected_rows')}}">

                            <span class="btn btn-danger">{{__('lang.Delete_multiple')}}</span>
                        </a>
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="editModalLabel">{{__('lang.Edit_data')}}</h5>
                                    ...
                                    <form id="editForm">
                                       {{csrf_field()}}
                                        <div class="form-group">
                                            <label for="name">{{__('lang.gia_ban')}}</label>
                                            <input type="text" class="form-control" id="gia_ban" name="gia_ban">
                                        </div>

                                        <div class="form-group">
                                            <label for="name">{{__('lang.Discounted_price')}}</label>
                                            <input type="text" class="form-control" id="gia_giam_gia" name="gia_giam_gia">
                                        </div>

{{--                                        <div class="form-group">--}}
{{--                                            <label for="name">Lợi nhuận</label>--}}
{{--                                            <input type="text" class="form-control" id="loi_nhuan" name="loi_nhuan">--}}
{{--                                        </div>--}}
                                    </form>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ __('lang.dong') }}</button>
                                    <button type="button" class="btn btn-success" onclick="editAllRecords()">{{__('lang.Send')}}</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
{{--                            @endforeach--}}
                            @foreach($module['list'] as $field)
                                ...
                                @php $count_sort++; @endphp
                            @endforeach
                            <th width="150px">{{__('lang.Thao_tac')}}</th>
                        </tr>
                        </thead>
                        ...
                        @foreach($listItem as $item)
                            <tr data-row="{{ $item->id }}" class="kt-datatable__row" style="left: 0px;">
                                ...
                                <td class="align-middle">
                                    ...
                                    <a href="/admin/{{ $module['code'] }}/delete/{{ $item->id }}" class="btn btn-danger">{{ __('lang.Xoa_bo') }}</a>

                                    <a href="javascript:void(0);"
                                       class="btn btn-primary btn-edit-single"
                                       data-id="{{ $item->id }}">
                                        {{ __('lang.edit') }}
                                    </a>
                                </td>
